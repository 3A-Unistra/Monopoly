 ...
 class Train_Station
 {
  ...
  class Program
  {
   public static string[] Board={"Start","Terrain","Community","Terrain","Tax","Train_Station","Terrain","Chance","Terrain","Terrain"
    ...
    public static Player[] opponents;
    ...
    public static void BuyTerrain(Player p,Terrain t)
    {   
        if(p.money>=t.price)
        {
            p.money -= t.price;
            t.owner=p.id;
            Console.WriteLine(p.name+" achète "+t.name+".");
        }
        else
            Console.WriteLine(p.name+" n'a pas assez d'argent pour acheter "+t.name+".");
    }

    public static int PayRent(Player p,Terrain t)
    {  
        int rentprice=0;
        if(t.nb_house==0)
        {
            rentprice=t.base_rent;
        }
        if(t.nb_house==1)
        {
            rentprice=t.house1_rent;
        }
        if(t.nb_house==2)
        {
            rentprice=t.house2_rent;
        }
        if(t.nb_house==3)
        {
            rentprice=t.house3_rent;
        }
        if(t.nb_house==4)
        {
            rentprice=t.house4_rent;
        }
        if(t.nb_house==5)
        {
            rentprice=t.hotel_rent;
        }
        if(rentprice<=p.money)
        {
            p.money -= rentprice;
            opponents[t.owner].money +=rentprice;
            return 0;
        }
        else
        {
            rentprice -=p.money;
            opponents[t.owner].money +=p.money;
            Console.WriteLine(p.name+" n'a pas assez d'argent pour payer "+t.name+".");
            p.money=0;
            return rentprice;
        }
    }    
      ...
      static void Main(string[] args)
    {
        ...
        opponents  = init_player(nb_player);
